...
then
	# valid compress
	if [ "${COMPRESSION}" = "lz4" ] || [ "${COMPRESSION}" = "zlib" ] || [ "${COMPRESSION}" = "lzma" ];
	then
		OPT_COMPRESSION="-C ${COMPRESSION}";
		# if COMPRESSION_LEVEL, check it is a valid regex
		# ignore it if this is lz4
		if [ ! -z "${COMPRESSION_LEVEL}" ] && [ "${COMPRESSION}" != "lz4" ];
		then
			if ! [[ "${COMPRESSION_LEVEL}" =~ ${REGEX_NUMERIC} ]];
			then
...
if [ ${DEBUG} -eq 1 ];
then
	echo "borg prune -v -s ${PRUNE_DEBUG} ${REPOSITORY} --keep-daily=${KEEP_DAYS} --keep-weekly=${KEEP_WEEKS} --keep-monthly=${KEEP_MONTHS}";
fi;
borg prune -v -s ${PRUNE_DEBUG} ${REPOSITORY} --keep-daily=${KEEP_DAYS} --keep-weekly=${KEEP_WEEKS} --keep-monthly=${KEEP_MONTHS} 2>&1 || echo "[!] Attic prune aborted";
